...
func TestWhatWeKnowAboutThePersonReachesTheSystemPrompt(t *testing.T) {
	...
	got := ComposeSystemPrompt(Profile{Prompt: "role"}, Record{}, roots{own: t.TempDir()}, state, nil)
...
func TestNoProfileAddsNothingToThePrompt(t *testing.T) {
	got := ComposeSystemPrompt(Profile{Prompt: "role"}, Record{}, roots{own: t.TempDir()}, t.TempDir(), nil)
