...
func TestBrowserProfilesGetTheSnapshotRuleInTheirPrompt(t *testing.T) {
	dir := t.TempDir()
	with := ComposeSystemPrompt(Profile{Browser: true}, Record{ID: "boss"}, roots{own: dir}, "", nil)
	without := ComposeSystemPrompt(Profile{Browser: false}, Record{ID: "boss"}, roots{own: dir}, "", nil)
