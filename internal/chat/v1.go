...
func (s *Server) v1Routes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /v1/mcp", s.apiGuard(s.listMCP))
	mux.HandleFunc("POST /v1/mcp", s.apiGuard(s.addMCP))
	mux.HandleFunc("PATCH /v1/mcp/{id}", s.apiGuard(s.updateMCP))
	mux.HandleFunc("DELETE /v1/mcp/{id}", s.apiGuard(s.removeMCP))
